...
export default function LocationCard({ location }) {
  ...
  const formatText = (text) => {
    ...
    return text.split('\n').map((line, index) => {
      ...
      if (trimmed === '') return <div key={index} className="location-spacer" />;
      ...
      if (trimmed.includes(':') && !trimmed.startsWith('http')) {
        ...
        if (label.toLowerCase() === 'whatsapp') {
          ...
          const waNumber = clean.length === 10 ? `52${clean}` : clean;
          const waMsg    = encodeURIComponent(`Hola, me interesa pedir información sobre la sucursal ${location.name}.`);
          mainWaLink = `https://wa.me/${waNumber}?text=${waMsg}`;
          
          return (
            <div key={index} className="wa-row">
              <span className="wa-label">{label}</span>
              <a href={mainWaLink} target="_blank" rel="noopener noreferrer" className="wa-link-button">
  ...
  const formattedContent = formatText(location.amenities);
  ...
  return (
    <div className="location-card-modern">
      ...
      <div className="location-content-modern">
        ...
        <div className="location-bottom-grid" style={!hasMap ? { gridTemplateColumns: '1fr' } : {}}>
